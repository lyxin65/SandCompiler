...
public class IRPrinter implements IIRVisitor {
    ...
    private String getBasicBlockName(BasicBlock bb) {
        if (!bbNames.containsKey(bb))
            bbNames.put(bb, "b" + String.valueOf(bbCount++) + (showId ? "(" + bb.id + ")" : ""));
    ...
    private String getVirtualRegsiterName(VirtualRegister virtualRegister) {
        if (!varNames.containsKey(virtualRegister))
            varNames.put(virtualRegister,
                    "v" + String.valueOf(varCount++) + (showId ? "(" + virtualRegister.id + ")" : ""));
    ...
    public void visit(Function function) {
        ...
        ArrayList<BasicBlock> reversePostOrder = new ArrayList<>(function.rPostOrder);
    ...
    public void visit(BinaryInst inst) {
        String op = null;
        if ((inst.op == BinaryInst.BinaryOp.ADD || inst.op == BinaryInst.BinaryOp.SUB) && inst.src instanceof Immediate
                && ((Immediate) inst.src).value == 0)
            return;
